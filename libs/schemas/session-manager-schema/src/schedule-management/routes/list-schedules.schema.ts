...
const LIST_SCHEDULES_SCHEMA = {
  description:
    'List session schedules for a room that are active within the given time range. Returns schedules where activeStart <= to and (activeEnd >= from or activeEnd is null).',
  ...
  querystring: Type.Object({
    ...
    from: Type.Optional(
      Type.String({
        format: 'date-time',
        description: 'Exclude schedules whose activeEnd is before this time.',
      }),
    ),
    to: Type.Optional(
      Type.String({
        format: 'date-time',
        description: 'Exclude schedules whose activeStart is after this time.',
      }),
    ),
  }),
  response: {
...
const LIST_SCHEDULES_ROUTE: BaseRouteDefinition = {
  method: 'GET',
  url: `${SESSION_MANAGER_BASE_PATH}/schedule-management/list-schedules`,
};
