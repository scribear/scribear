...
const END_SESSION_EARLY_SCHEMA = {
  description:
    'End a currently-active non-AUTO session before its scheduled end by setting endOverride to now. The gap left by the early end is reconciled into AUTO sessions where covered by an active window.',
...
const END_SESSION_EARLY_ROUTE: BaseRouteDefinition = {
  method: 'POST',
  url: `${SESSION_MANAGER_BASE_PATH}/schedule-management/end-session-early`,
};
