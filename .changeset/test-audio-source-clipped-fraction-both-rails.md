---
'@scribear/test-audio-source': patch
---

Count both rails in `clippedFraction`, and cover the DSP and fault engines with
...
saturated *negative* peak lands on **-32767** and only reaches -32768 once it is a
...
the one it warns against. The cause is that `hardClipToRail` is a *gain* rather
...
reports reads *below* the `clipPct` that was set — at `clipPct: 90` with
