---
'@scribear/base-api-client': minor
---

The API client no longer rejects when a declared status arrives with a body
that is not JSON.
...
hatch into the parse. But a declared status is no guarantee the *service*
