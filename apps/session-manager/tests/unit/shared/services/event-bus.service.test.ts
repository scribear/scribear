...
describe('EventBusService', () => {
  ...
  describe('subscribe', (it) => {
    ...
    it('removes the channel entry once the last listener unsubscribes', () => {
      // Arrange
      const listener = vi.fn();
      const unsubscribe = bus.subscribe(TEST_CHANNEL, listener, 'a');
      ...
      // Assert - the second publish reaches no listeners and does not throw.
    ...
    it('is idempotent when called more than once', () => {
      ...
      // Assert - publishing reaches no listeners and the double-unsubscribe doesn't throw.
      bus.publish(TEST_CHANNEL, { value: 1 }, 'a');
      expect(listener).not.toHaveBeenCalled();
    });
  });
